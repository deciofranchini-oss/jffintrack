...
// ── App Logo (Supabase Storage) ─────────────────────────────────────────
async function uploadAppLogoToStorage(file){
  if(!file) throw new Error('no file');
  if(!window.supabase) throw new Error('supabase not ready');
  const ext = (file.name || 'logo.png').split('.').pop().toLowerCase();
  const safeExt = ['png','jpg','jpeg','webp','svg'].includes(ext) ? ext : 'png';
  const family = (typeof famId==='function' ? famId() : 'global') || 'global';
  const path = `app/logo/${family}/logo.${safeExt}`;
...
async function saveAppLogoFromSettings(){
  ...
  try{
    ...
    if(!url){ toast('Falha ao obter URL pública do logo.','error'); return; }
    await saveAppSetting('app_logo_url', url);
    try{ localStorage.setItem('app_logo_url', url); }catch(e){}
    if(typeof setAppLogo==='function') setAppLogo(url);
    toast('Logotipo atualizado.','success');
  }catch(e){
    console.error(e);
    toast('Erro ao enviar logotipo. Verifique o bucket/permite acesso público.','error');
  }
}
try{ window.saveAppLogoFromSettings=saveAppLogoFromSettings; }catch(e){}
